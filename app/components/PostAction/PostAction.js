...
export default class PostAction extends Component {
  ...
  onPress = (onPress) => {
    ...
    const decider = this.state.wasPressed ? this.state.user : this.props.actionUser;
    const counter = this.state.wasPressed ? this.state.count : this.props.actionCount;

    this.setState(previousState => ({
      wasPressed: true,
      user: previousState.wasPressed ? !previousState.user : !this.props.actionUser,
  ...
  render() {
    const count = this.state.wasPressed ? this.state.count : this.props.actionCount;
      ...
      <TouchableOpacity onPress={() => this.onPress(this.props.onPress)}>
        <View style={styles.container}>
          <View style={styles.iconBox}>
            <PostActionIcons name={this.props.actionType} size={16} color={iconColor} />
          </View>
