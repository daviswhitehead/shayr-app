...
describe('cloud functions', () => {
  before((done) => {
    ...
    ].reduce((acc, val) => acc.concat(val))).then((value) => {
      done()
    });
    ...
    Promise.all([
      db.collection('users').get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          db.collection('users').doc(doc.id).collection('shares').get().then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
              doc.ref.delete();
            });
          });
      ...
      db.collection('posts').get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          db.collection('posts').doc(doc.id).collection('shares').get().then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
              doc.ref.delete();
            });
          });
  ...
  describe('writtenUserAdd', () => {
    ...
    it('should update associated post counter', async () => {
      ...
      const originalPost = await db.doc(postPath).get().then((doc) => {
        return doc.data()
      }).catch((e) => {
        return e
      });
      ...
      const updatedPost = await db.doc(postPath).get().then((doc) => {
        return doc.data()
      }).catch((e) => {
        return e
      });
